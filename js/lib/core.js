...
;(function () {
    ...
    M.util = {
        ...
        doLogin: function () {
            ...
            var rediect = location.href;
            var url = '/login.html?redirect=' + encodeURIComponent(rediect);
            location.replace(url);
        },

        doLogout: function () {
            M.ajax({
                url: '/anti_fraud/usr/logout',
                dataType: 'json',
                type: 'POST',
        ...
        getShowPageKey: function(){
            var key = M.util.getParam('cpage', location.hash) || M.util.getParam('cpage', location.search)||'';
            return key;
        },

        formateDispatch: function (dispatch_type) {
            var str = '';
            if(dispatch_type == 1 ){
                str = M.util.addSucc('邀请');
            }else if(dispatch_type == 0){
                str = M.util.addDef('未派单');
            }else if(dispatch_type == 2){
                str = M.util.addInfo('派单');
            }else{
                str = M.util.addErr('未知'+ dispatch_type);
            }
            return str;
        },

        formateUserStatus: function (user_status) {
            //0:待实名认证 -3:待面签 3:待初审 2:待终审 -2:初审拒绝 6:初审退回 7:终审退回 -1:终审拒绝 1:已授信
            var str = '';
            if(user_status == 0 ){
                str = M.util.addDef('待实名认证');
            }else if(user_status == -3){
                str = M.util.addInfo('待面签');
            }else if(user_status == 3){
                str = M.util.addInfo('待初审');
            }else if(user_status == 2){
                str = M.util.addInfo('待终审');
            }else if(user_status == -2){
                str = M.util.addErr('初审拒绝');
            }else if(user_status == 6){
                str = M.util.addWarn('初审退回');
            }else if(user_status == 7){
                str = M.util.addWarn('终审退回');
            }else if(user_status == -1){
                str = M.util.addErr('终审拒绝');
            }else if(user_status == 1){
                str = M.util.addSucc('已授信');
            }else{
                str = M.util.addErr('未知'+ user_status);
            }
            return str;
        },

        formateTradeType: function (type) {
            //操作类型： 1:借款 2:主动还款 3:自动还款 4:绑卡
            var str = '';
            if(type == 1 ){
                str = '借款';
            }else if(type == 2){
                ...
                str = M.util.addErr('未知'+ type);
            }
            return str;
        },

        formateTradeStatus: function(type){
            //1:借款成功 2:借款失败 3:还款成功 4:还款失败 5:绑卡成功 6:绑卡失败 7:借款中 8:还款中 9:绑卡中
            var str = '';
            if(type == 1 ){
                str = M.util.addSucc('借款成功');
            }else if(type == 2){
                str = M.util.addErr('借款失败');
            }else if(type == 3){
                str = M.util.addSucc('还款成功');
            }else if(type == 4){
                str = M.util.addErr('还款失败');
            }else if(type == 5){
                str = M.util.addSucc('绑卡成功');
            }else if(type == 6){
                str = M.util.addErr('绑卡失败');
            }else if(type == 7){
                str = M.util.addWarn('借款中');
            }else if(type == 8){
                str = M.util.addWarn('还款中');
            }else if(type == 9){
                str = M.util.addWarn('绑卡中');
            }else{
                str = M.util.addErr('未知'+ type);
            }
            return str;
        ...
        formateAuditStatus: function(type){
            var str = '';
            if(type == 1 ){
                str = M.util.addSucc('审核通过');
            }else if(type == 2){
                str = M.util.addErr('审核拒绝');
            }else{
                str = M.util.addErr('未知'+ type);
            }
            return str;
        ...
        checkMobile: function (phoneno, callback) {
...
;(function () {
    ...
    var config = {
        urls: {
            getPublicKey:'/anti_fraud/get_pub_key'
        }
    };
